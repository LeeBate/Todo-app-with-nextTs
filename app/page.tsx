  ...
  const handleDrop = async (todoId: number, newCompleted: boolean) => {
    try {
      ...
      if (!res.ok) {
       ...
       setTodos((prev) => {
        const updatedTodos = prev.map((t) => (t.id === todoId ? { ...t, completed: newCompleted } : t))
        ...
        if (draggedTodo) {
          if (newCompleted) {
            ...
            const otherCompletedTodos = completedTodos.filter((t) => t.id !== todoId)
            return [...pendingTodos, draggedTodo, ...otherCompletedTodos]
          } else {
            // ถ้าลากไป "ยังไม่เสร็จ" ให้ไปอยู่ด้านบนของ pending todos
            const otherPendingTodos = pendingTodos.filter((t) => t.id !== todoId)
